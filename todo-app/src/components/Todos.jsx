...
import { removeTodo, editTodo } from '../actions'; // assuming you have these actions defined
const Todos = () => {
    ...
    return todos.map(todo => (
      <li key={todo.id} className="flex items-center mb2">
        <span className={`mr2 ${todo.completed ? 'strike' : ''}`}>{todo.text}</span>
            <button className="f6 link dim ph3 pv2 mb2 dib white bg-green" onClick={() => handleComplete(todo.text)}>Complete</button>
